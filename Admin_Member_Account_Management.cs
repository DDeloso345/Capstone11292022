...
namespace Capstone
{
    public partial class Admin_Member_Account_Management : Form
    {
        ...
        public Admin_Member_Account_Management()
        {
            InitializeComponent();
            InsertNotifs();
        }

        public void Admin_Member_Account_Management_Load(object sender, EventArgs e)
        {
        ...
        public void ComboBoxSel()
        {
            ...
            //organizationtxt.DataSource = orc;
            //organizationtxt.DisplayMember = "SchoolOrOrganization";
        }
        public void clear()
        {
            ...
            else if (uidtxt.Text.Length > 0 && contactnotxt.Text.Length > 0)
            {
                try
                {
                    ...
                    if (test.Count == 0 && memberstart_txt.Text.Equals(Properties.Settings.Default.memberstart) && contactnotxt.Text.StartsWith("09") && contactnotxt.TextLength == 11 && uidtxt.TextLength == 14)
                    {
                        ...
                        {
                            //int num = organizationtxt.Items.IndexOf(organizationtxt.Text) + 1;
                            ComboBoxSel();
                            Properties.Settings.Default.memberuid = uidtxt.Text;
                            Properties.Settings.Default.memberfirstname = firstnametxt.Text;
                            Properties.Settings.Default.memberlastname = lastnametxt.Text;
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            if (uidtxt.Text.Length == 0 || firstnametxt.Text.Length == 0 || middlenametxt.Text.Length == 0 || lastnametxt.Text.Length == 0 || organizationtxt.Text.Length == 0 || addresstxt.Text.Length == 0 || emailtxt.Text.Length == 0 || contactnotxt.Text.Length == 0 || memberstart_txt.Text.Length == 0 || memberend_txt.Text.Length == 0 || imgpathtxt.Text.Length == 0)
            {
                MessageBox.Show("One or more of the fields have been left blank.\nPlease fill in the blank fields to successfully delete a library member account.", "Warning!", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else if (uidtxt.Text.Length > 0 && contactnotxt.Text.Length > 0)
            {
                try
                {
                    ...
                    String email = emailtxt.Text;
                    String numb = contactnotxt.Text;
                    String st = memberstart_txt.Text;
                    String end = memberend_txt.Text;
                    var dlg = MessageBox.Show("Please check the following information for any errors\nbefore proceeding to delete the specified record.\n\n" + "\nUID: COCPLM" + uid + "\nFirst Name: " + fn + "\nMiddle Name: " + mn + "\nLast Name: " + ln + "\nSuffix: " + suf + "\nAddress: " + add + "\nOrganization: " + org + "\nE-mail Address: " + email + "\nContact Number: " + numb + "\nMembership Start: " + st + "\nMembership End: " + end + "\n\nAre the following information correct?", "Confirmation for member account deletion", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
                    if (dlg == DialogResult.Yes)
                    {
                        acc_cmd.DeleteMember(uidtxt.Text, idlbl.Text);
                        UpdateBinding();
                        clear();
                    }
                    else
        ...
        private void orhistory_Click(object sender, EventArgs e)
        {
            try
            {
                if (Properties.Settings.Default.memberid.Equals("[ID Txt]"))
                {
                    Properties.Settings.Default.memberid = "1";
                    Properties.Settings.Default.Save();
                }
                else
                {
        ...
        private void memberstart_txt_ValueChanged(object sender, EventArgs e)
        {
        ...
        {
            /*if (selaction.SelectedIndex == 0) { 
            
            }
            else if (selaction.SelectedIndex == 1) {
                memb.CompareSameOrg(organizationtxt.Text);
                ComboBoxSel();
                selaction.SelectedIndex = 0;
            }
            else if (selaction.SelectedIndex == 2) {
                memb.UpdateOrg(organizationtxt.Text, orgid.Text);
                ComboBoxSel();
                selaction.SelectedIndex = 0;
            }
            else if (selaction.SelectedIndex == 3) {
                memb.DeleteOrg(organizationtxt.Text, orgid.Text);
                ComboBoxSel();
                selaction.SelectedIndex = 0;
            }
            */
        ...
        private void orgid_Click(object sender, EventArgs e)
        {

        }

        private void notifpnl_Click(object sender, EventArgs e)
        {
        ...
        private void notifdgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (notifdgv.Rows.Count >= 0)
                {
                    ...
                    var del = MessageBox.Show("The following notification has the message:" + "\n\n" + memb.GetMsgNotif(notif).ToString() + "\n\nThis notification was posted on " + memb.GetTimeStamp(notif) + "." + "\n\nWould you like to discard the following notification?", "Notice", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                    if (del == DialogResult.Yes)
                    {
                        memb.DeleteNotif(notif);
                        UpdateBinding();
                    }
                }
            }
        ...
        private void organizationtxt_KeyDown(object sender, KeyEventArgs e)
        {
            /*SearchOrg();
        ...
        private void organizationtxt_TextChanged(object sender, EventArgs e)
        {
        }
        ...
        {
            membership_timer.Start();
            SelectOrganization_Admin a = new SelectOrganization_Admin();
            a.Show();
        }
