...
namespace Capstone
{
    public partial class Admin_BookBorrowing : Form
    {
        ...
        private void Admin_BookBorrowing_Load(object sender, EventArgs e)
        {
            UpdateBinding();
            ComboBoxSel();
        }
        ...
        private void bktitle_SelectedIndexChanged(object sender, EventArgs e)
        {
            titleid.Text = bc.getBKTitleID(bktitle.Text);
            accno.Text = bc.setAccNo(titleid.Text);
            bkauthor.Text = bc.setAuthor(titleid.Text);
        }

        private void accno_SelectedIndexChanged(object sender, EventArgs e)
        {
        }

        private void bkauthor_SelectedIndexChanged(object sender, EventArgs e)
        {
            autid.Text = bc.getAuthorID(bkauthor.Text);
        }

        private void cocpluid_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        {
            /*
            empunid.Text = bc.getcurrEmployeeID(approvedby.Text);
            empname.Text = bc.setEmpName(empunid.Text);
            */
        }
        ...
        private void insertbtn_Click(object sender, EventArgs e)
        {
                ...
                cocpluid.Text.Length == 0)
            {
                ...
                + "\nDue Date: " + duedt.Value.ToShortDateString() + "\n\nAre the following information correct?", "Confirmation for adding book borrowing record entry", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk);
        ...
        {
            bc.DeleteBKBRRecords(bookbrid.Text);
            UpdateBinding();
            ComboBoxSel();
            clear();
        }
        ...
        {
            Admin_BKBR_ApprovedNotifs a = new Admin_BKBR_ApprovedNotifs();
            a.Show();
        }
        ...
        {
            Admin_BKBorrowersInfo ad = new Admin_BKBorrowersInfo();
            ad.Show();
        }
        ...
        private void notifdgv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (notifdgv.Rows.Count >= 0)
                {
                    ...
                    //if (notif.StartsWith("A member with the following UID"))
                    //{
