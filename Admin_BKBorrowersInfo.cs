...
namespace Capstone
{
    public partial class Admin_BKBorrowersInfo : Form
    {
        ...
        public Admin_BKBorrowersInfo()
        {
            InitializeComponent();
        }

        private void Admin_BKBorrowersInfo_Load(object sender, EventArgs e)
        {
            UpdateBinding();
            ComboBoxSel();
        }
        ...
        private void dgv_bkbr_ind_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (dgv_bkbr_ind.SelectedRows.Count >= 0)
                {
                    ...
                    + "\nApproved By: " + appby
                    + "\nApproved On: " + appon
                    + "\n\nWould you like to delete the specified record?"
                    , "Notice", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                    if (del == DialogResult.Yes)
                    {
                        br.DeleteBKBR_Ind_Rec(id);
                    }
                }
            }
            catch (Exception) { }
