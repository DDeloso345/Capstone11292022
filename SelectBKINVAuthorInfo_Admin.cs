...
namespace Capstone
{
    public partial class SelectBKINVAuthorInfo_Admin : Form
    {

        SQLBookInventoryCommandsClass t = new SQLBookInventoryCommandsClass();
        List<getAuthorInfo> bmc = new List<getAuthorInfo>();
        public SelectBKINVAuthorInfo_Admin()
        {
            InitializeComponent();
        }

        private void SelectBKINVAuthorInfo_Admin_Load(object sender, EventArgs e)
        {
            UpdateBinding();
        ...
        {
            List<getBKInvColumns> bi = new List<getBKInvColumns>();
            t.DeleteAuth(autnotxt.Text, auttxt.Text, orgid.Text);
            UpdateBinding();
        }
