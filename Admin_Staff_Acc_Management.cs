...
namespace Capstone
{
    public partial class Admin_Staff_Acc_Management : Form
    {
        public Admin_Staff_Acc_Management()
        {
            InitializeComponent();
        }
        ...
        private void Admin_Staff_Acc_Management_Load(object sender, EventArgs e)
        {
            UpdateBinding();
            cmbbox_criteria.SelectedIndex = 0;
            positioncombobox.SelectedIndex = 0;
            hierarchylvltxt.SelectedIndex = 1;
        ...
        {
            acc = staff_cmd.LoadStaffData_Admin();
        ...
        private void searchbtn_Click(object sender, EventArgs e)
        {
            ...
            {
                selection = "FirstName";
                acc = acc_cmd.SearchStaff_Admin(selection, inputsearchbar.Text);
                dgv_admin_acc_tbl.DataSource = acc;
            }
            ...
            {
                acc = acc_cmd.SearchStaff_Admin("Suffix", inputsearchbar.Text);
                dgv_admin_acc_tbl.DataSource = acc;
            }

            else if (cmbbox_criteria.Text.Equals("Position"))
            {
                acc = acc_cmd.SearchStaff_Admin("Position", inputsearchbar.Text);
                dgv_admin_acc_tbl.DataSource = acc;
            }
            ...
            {
                acc = acc_cmd.SearchStaff_Admin(cmbbox_criteria.Text, inputsearchbar.Text);
                dgv_admin_acc_tbl.DataSource = acc;
            }

            else if (cmbbox_criteria.Text.Equals("Active Status"))
            {
                acc = acc_cmd.SearchStaff_Admin("ActiveStatus", inputsearchbar.Text);
                dgv_admin_acc_tbl.DataSource = acc;
            }
            ...
            if (cmbbox_criteria.SelectedIndex == 1)
            {
                ...
                acc = acc_cmd.SearchStaff_Admin(selection, inp);
                dgv_admin_acc_tbl.DataSource = acc;
            }
                ...
                try
                {
                    ...
                    if (hierarchylvltxt.Text.Equals("Administrator"))
                    {
                        ...
                        if (positioncombobox.Text.Equals("Librarian III"))
                        {
                            ...
                            if (dlg == DialogResult.Yes)
                            {
                                var test = acc_cmd.Upd_CompareSameUsernameStaff(idlbl.Text, usernametxt.Text);
                                if (test.Count == 0)
                                {
                                    if (suffixtxt.Text.Equals("N/A"))
                                    {
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("One or more of the fields have been left blank.\nPlease fill in the blank fields to successfully delete a staff account.", "Warning!", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else if (contactnotxt.TextLength > 0)
            {
                try
                {
                    ...
                    DialogResult dialog = MessageBox.Show("Please check the following information if it is to be deleted.\n\nFirst Name: " + fn + "\nMiddle Name: " + mn + "\nLast Name: " + ln + "\nString: " + suf + "\nPosition: " + pos + "\nHierarchy Level: " + hier + "\nE-mail Address: " + email + "\nContact Number: " + numb + "\nUsername: " + un + "\nPassword: " + pw + "\n\nAre the following information correct?", "Confirmation for staff account deletion", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
