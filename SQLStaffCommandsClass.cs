...
namespace Capstone
{
    public class SQLStaffCommandsClass
    {
        public List<AccountDetails_Get_Staff> LoadStaffData()
        {
            ...
            {
                var output = connection.Query<AccountDetails_Get_Staff>($"SELECT emp.id, emp.FirstName, emp.MiddleName, emp.LastName, s.Suffix, ep.Position, er.EmpRoles, emp.EMailAddress, emp.ContactNo, emp.Username, emp.ImgPath, es.ActiveStatus "
